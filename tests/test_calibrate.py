...
def test_weighted_pearson_matches_pearson_with_uniform_weights():
    ...
    assert M.weighted_pearson(gt, pred, _Ctx(), exp=2.0) == pytest.approx(M.pearson(gt, pred, None))
...
def test_weighted_r2_matches_r2_with_uniform_weights():
    ...
    assert M.weighted_r2(gt, pred, _Ctx(), exp=2.0) == pytest.approx(M.r2(gt, pred, None))
